...
import { Users, UserCheck, Briefcase, CalendarCheck, Wallet, Bell } from "lucide-react";
...
function useStats() {
  return useQuery({
    ...
    queryFn: async () => {
      const [emp, active, applicants, leaves, payroll, interviews, attendance] = await Promise.all([
        ...
        supabase.from("interviews").select("id,scheduled_at,applicant_id,type,status,applicants(first_name,last_name)").gte("scheduled_at", new Date().toISOString()).order("scheduled_at").limit(5),
        supabase.from("attendance").select("date,hours_worked").gte("date", new Date(Date.now() - 6*86400000).toISOString().slice(0,10)),
      ]);

      const totalPayroll = (payroll.data ?? []).reduce((s, r) => s + Number(r.net_salary || 0), 0);
...
function useDeptDistribution() {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("employees").select("department_id,departments(name)");
      ...
      <CardContent className="p-5">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-3xl font-bold" style={{ fontFamily: "Space Grotesk" }}>{value}</p>
...
function Dashboard() {
  ...
  return (
    <div>
      <PageHeader title="Dashboard" description="Real-time overview of your organization" />

      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {isLoading ? Array.from({length: 4}).map((_,i) => <Skeleton key={i} className="h-28" />) : (
          <>
            <StatCard icon={Users} label="Total Employees" value={data?.totalEmployees} />
            <StatCard icon={UserCheck} label="Active Employees" value={data?.activeEmployees} />
            <StatCard icon={Briefcase} label="Applicants" value={data?.applicants} />
            <StatCard icon={CalendarCheck} label="Pending Leaves" value={data?.pendingLeaves} />
          </>
        )}
      </div>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>Daily check-ins across the organization</CardDescription>
          </CardHeader>
          <CardContent className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data?.attendanceSeries ?? []}>
                <XAxis dataKey="day" stroke="hsl(215 25% 47%)" fontSize={12} />
                <YAxis stroke="hsl(215 25% 47%)" fontSize={12} allowDecimals={false} />
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><CalendarCheck className="h-4 w-4" /> Upcoming Interviews</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <ul className="space-y-3">
                {data.upcomingInterviews.map((i: any) => (
                  <li key={i.id} className="flex items-center justify-between rounded-lg border border-border p-3">
                    <div>
                      <p className="font-medium">{i.applicants?.first_name} {i.applicants?.last_name}</p>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="rounded-lg border border-border p-3">
              ...
              <p className="text-xs text-muted-foreground">Your HR workspace is ready. Explore the modules from the sidebar.</p>
            </div>
            <div className="rounded-lg border border-border p-3">
              <p className="font-medium">Payroll ledger</p>
              <p className="text-xs text-muted-foreground">Total processed: ₱{(data?.totalPayroll ?? 0).toLocaleString()}</p>
            </div>
          </CardContent>
        </Card>
      </div>
